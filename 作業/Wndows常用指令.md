...
NETSTAT [-a] [-b] [-e] [-f] [-n] [-o] [-p proto] [-r] [-s] [-x] [-t] [interval]
  ...
  -p proto      顯示由 proto 指定的通訊協定連線; proto 可以是 下列任一個:
  ...
  -s            顯示每個通訊協定的統計資料。依預設，將會顯示 IP、IPv6、
                ICMP、ICMPv6、TCP、TCPv6、UDP 和 UDPv6 的統計 資料; 可以使用
                -p 選項來指定預設的子集。
  -t            顯示目前的連線卸載狀態。
  -x            顯示 NetworkDirect 連線、接聽程式與共用端點。
  -y            顯示所有連線的 TCP 連線範本。不能合併使用其他 選項。
...
```
```
netstat是WINDOWS作業系統所提供,用來顯示通訊協定統計資料和目前的 TCP/IP 網路連線.
請問要顯示所有連線和接聽連接埠時須使用列哪一個參數?
(A)-a(B)-n(C)-o(D)-r
```
```
